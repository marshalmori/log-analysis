...
class Erros:
    ...
    com o objetivo de agrupar os valores das datas e a quantidade de
    requisições HTTP, separando as que foram bem sucedidas das que tiveram
    erro (404 Not Found). Ja o método select_erros_percentagem serve para
    selecionar dentre as datas aquela que apresenta erro 404 com porcentagem
    maior que 1%.
    '''
    ...
    def create_tb_estatus_erros(cls):
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute('''create or replace view tb_status_erros as select
                              to_char(date(time), 'Month DD, YYYY') as
                              mes_dia_ano, count(status) as status_erros
                              from log where status='404 NOT FOUND' group
                              by date(time) order by date(time)''')
    ...
    def select_erros_percentagem(cls):
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute('''select mes_dia_ano,
                              ...
                              (cast(status_erros as float8)/cast(status_all
                              as float8)*100) > 1  order by mes_dia_ano''')
