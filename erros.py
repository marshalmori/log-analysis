...
# create or replace view tb_status_erros as select to_char(date(time), 'Month DD, YY') as mes_dia_ano, count(status) as status_erros from log where status='404 NOT FOUND' group by date(time) order by date(time);
# create or replace view tb_status_all as select to_char(date(time), 'Month DD, YY') as m_d_a, count(status) as status_all from log group by date(time) order by date(time);
